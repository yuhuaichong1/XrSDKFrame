...
{
    private Dictionary<int, ConfLevel> _dataMap;
    private List<ConfLevel> _dataList;
    
    public TBLevel(ByteBuf _buf)
    {
        _dataMap = new Dictionary<int, ConfLevel>();
        _dataList = new List<ConfLevel>();
        
        for(int n = _buf.ReadSize() ; n > 0 ; --n)
        {
            ConfLevel _v;
            _v = ConfLevel.DeserializeConfLevel(_buf);
            _dataList.Add(_v);
    ...
    public Dictionary<int, ConfLevel> DataMap => _dataMap;
    public List<ConfLevel> DataList => _dataList;

    public ConfLevel GetOrDefault(int key) => _dataMap.TryGetValue(key, out var v) ? v : null;
    public ConfLevel Get(int key) => _dataMap[key];
    public ConfLevel this[int key] => _dataMap[key];
    ...
    {
        var reloadMap = new TBLevel(_buf);
