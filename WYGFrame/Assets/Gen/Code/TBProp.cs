...
{
    private Dictionary<int, ConfProp> _dataMap;
    private List<ConfProp> _dataList;
    
    public TBProp(ByteBuf _buf)
    {
        _dataMap = new Dictionary<int, ConfProp>();
        _dataList = new List<ConfProp>();
        
        for(int n = _buf.ReadSize() ; n > 0 ; --n)
        {
            ConfProp _v;
            _v = ConfProp.DeserializeConfProp(_buf);
            _dataList.Add(_v);
    ...
    public Dictionary<int, ConfProp> DataMap => _dataMap;
    public List<ConfProp> DataList => _dataList;

    public ConfProp GetOrDefault(int key) => _dataMap.TryGetValue(key, out var v) ? v : null;
    public ConfProp Get(int key) => _dataMap[key];
    public ConfProp this[int key] => _dataMap[key];
