...
public abstract class UIViewBehaviourEditor : Editor
{
    ...
    protected string buttonFuncContent;

    protected virtual void OnEnable()
    {
        buttonFuncContent = string.Empty;
    ...
    private void GenerateUIBaseCode()
    {
        ...
        string content = UIEditorUtility.kUIBaseCode.Replace("${ClassName}", className);
        content = content.Replace("${namespace}", "XrCode");
        content = content.Replace("${ButtonFuncContent}", buttonFuncContent);
        content = content.Replace("${BaseClassName}", "BaseUI");
        ...
        //int retCode = GetExportBaseCodeStrs(out variantsDefineStr, out bindCompsStr);
        //if (retCode < 0) { return; }
    ...
    private void GenerateUITempCode()
    {
        ...
        string subPath = Path.GetRelativePath(fullRootDir, fullPrefabPath);
        string className = Path.GetFileNameWithoutExtension(subPath);
        string subSavePath = Path.Combine($"{Path.GetDirectoryName(subPath)}/UITemplate", className + "_Temp.cs");
        ...
        string variantsDefineStr, bindCompsStr, bindEventStr, unBindEventStr, buttonFuncStr;
        int retCode = GetExportBaseCodeStrs(out variantsDefineStr, out bindCompsStr, out bindEventStr, out unBindEventStr, out buttonFuncStr);
        buttonFuncContent = buttonFuncStr;
        if (retCode < 0) { return; }
        content = content.Replace("${VariantsDefine}", variantsDefineStr + (!string.IsNullOrEmpty(variantsDefineStr) ? "\r" : string.Empty));
        content = content.Replace("${BindComps}", bindCompsStr);
        content = content.Replace("${BindEvent}", bindEventStr);
        content = content.Replace("${UnBindEvent}", unBindEventStr);
    ...
    private int GetExportBaseCodeStrs(out string variantsDefineStr, out string bindCompsStr, out string bindEventsStr, out string unbindEventsStr, out string buttonFuncStr)
    {
        ...
        string variantsDefineTempalte = "protected ${CompType} m${GoName};";
        //string bindCompsLine1 = "m_${GoName}_${CompName} = (${CompType})viewBehaviour.GetComponentByIndexs(${i}, ${j});";
        string bindCompsLine = "m${GoName} = mTransform.Find(\"${CompPath}\").GetComponent<${CompName}>();";
        string bindEventsLine = "m${GoName}.onClick.AddListener( On${GoName}ClickHandle);";
        string unbindEventsLine = "m${GoName}.onClick.RemoveAllListeners();";
        string buttonFuncLine = "private void On${GoName}ClickHandle(){}";
        ...
        for (int i = 0; i < uwb.opElementList.Count; i++)
        {
            ...
            {
                StringBuilder compPath = GetElementPath(opElement.target, new StringBuilder());
                ...
                //string bindCompsLine = "m_${GoName} = t.FindChild($(CompPath)).GetComponent<${CompType}>()";

                string bcLine = new string(bindCompsLine);
                bcLine = bcLine.Replace("${GoName}", formatedGoName);
                bcLine = bcLine.Replace("${CompPath}", compPath.ToString());
                bcLine = bcLine.Replace("${CompName}", compType);
                ...
                if (compType.Equals("Button"))
                {
                    ...
                    //按钮函数
                    string bfLine = new string(buttonFuncLine);
                    bfLine = bfLine.Replace("${GoName}", formatedGoName);
                    bfsb.Append("\r\t").Append(bfLine);
                }
        ...
        buttonFuncStr = bfsb.ToString();

        return 0;
    }

    private StringBuilder GetElementPath(GameObject obj, StringBuilder path)
    {
        path.Insert(0, obj.name);
        Transform t = obj.transform.parent;
        if (t.name.Equals(this.target.name))
        {
            return path;
        }
        path.Insert(0, "/");
        return GetElementPath(t.gameObject, path);
    }
