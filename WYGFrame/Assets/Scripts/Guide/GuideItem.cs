...
public class GuideItem
{
    ...
    public List<string> diglogContent;              //提示框内容（对应Language表）
    public List<string> diglogPos;                  //提示框位置
    public List<string> handPos;                    //手位置
    public bool ifMask;                             //是否有黑色遮罩
    public List<string> transparentPos;             //透明框位置
    public List<string> clickPos;                   //教程可点击部分
    public Dictionary<string, string> extraStart;   //开始时的额外字段
    public Dictionary<string, string> extraEnd;     //结束时的额外字段
}
